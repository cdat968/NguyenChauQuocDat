...
public class Guerrillamail extends Utilities {
	...
	public HomePage generateRandomEmail(String str) {
		WebDriverWait wait = new WebDriverWait(Constant.WEBDRIVER, Duration.ofSeconds(3));
		
		if (this.getElement(checkScrambleAddress).isSelected()) {
			this.getElement(checkScrambleAddress).click();
		}
		
		this.getElement(buttonClickToEdit).click();
		
		this.getElement(_txtEmailInput).clear();
		this.getElement(_txtEmailInput).sendKeys(str);
		this.getElement(_btnSaveEmail).click();
		wait.until(ExpectedConditions.visibilityOfElementLocated(_txtSuccessMsg));
		this.getElement(_txtEmail).getText();
